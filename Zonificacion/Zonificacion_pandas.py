...
def _mejorarDatos(dfin):
    ...
    def remove_whitespace(x):
        try:
            # quitar espacios de más y quitar **********
            x=' '.join(str(x).split()) #quitar todos los espacios de más
            x = x.replace('**********','')
        except:
            pass        
        return x
    
    dfinter = dfin.drop(dfin.columns[dfin.dtypes=='float'], axis=1)
    dfinter = dfinter.applymap(remove_whitespace)
    dfin.update(dfinter)
    dfin.replace(r'^\s*$', np.NaN, regex=True, inplace=True)  # Vacios y solo espacios son NaN    
    dfin.dropna(how='all', subset=['Owner', 'Name'], thresh=2, inplace=True) # Eliminar filas vacias
...
def _cruceDataZonas(dfin, zonas):
    '''
    ...
    dfin['Zona'] = 'NA'
    zonas['comp'] = [[] for i in zonas.index]
...
def concatenarData(inputFiles, zonasFile):
    '''
    ...
    dataDf, zonasDf = _getData(inputFiles, zonasFile)

    dataDf = _mejorarDatos(dataDf)
    dataDf = _agregarInfo(dataDf)  # mejorar la info
    dataDf, ZonasDf = _cruceDataZonas(dataDf, zonasDf)  # cruzar data con zonas
...
def _getData(inputFiles, zonasFile):
    
    Reps = pd.read_csv(inputFiles, header=None, squeeze=True, dtype=str)    
    NA = ['**********', '', '<Unset>']
    dataDf = pd.concat([pd.read_excel(i, header=1, na_values=NA, index_col=None) for i in Reps])
    dataDf.reset_index(drop=True, inplace=True)
    zonasDf = pd.read_csv(zonasFile, sep='\t')
    
    return dataDf, zonasDf
